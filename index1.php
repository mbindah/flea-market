...
if (!isset($_SESSION['vendname'])) {
    header("Location: index.html");
}else{
  $filter = $_SESSION['vendname'];
...
<html lang="en">
    ...
    <meta name="viewport" content="width=device-width, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no">
     ...
     <!-- Site Metas -->
    <title>Flea Market System - Vendor Homepage</title>  
        ...
        <nav class="megamenu navbar navbar-default">
            <div class="container">
                <div class="navbar-header">
                    ...
                    <a class="navbar-brand" href="index1.php"><img src="images/logos/logo.png" alt="image"></a>
                </div>
...
$sql = "SELECT `Product_ID`, `Name`, `Details`, `Image`, `Market_Name`, `Created_At`, `Updated_At`, `Status` FROM `products` WHERE `User_ID` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
...
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure you want to delete this product?')?window.location.href='insertion.inc.php?action=deleteP&id=<?php echo($row["Product_ID"]); ?>':true;" title='Delete Product'>Delete</button></td>
</tr>
<?php
}
} else { echo "No results"; }
...
$sql = "SELECT `comments`.`Comment_ID`, `comments`.`Product_ID`, `comments`.`Content`, `comments`.`Created_At`, `users`.`Fullname` FROM `comments` JOIN `users` ON `comments`.`User_ID` = `users`.`User_ID` WHERE `users`.`User_ID` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
            ...
            <div class="row">
                <div class="col-md-12" id="register">
                    <div class="contact_form">
                        <div id="message"></div>
                        <form class="row" action="insertion.inc.php" name="contactform" method="post" enctype="multipart/form-data">
                            <fieldset class="row-fluid">
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                    <input type="text" required name="pname" id="" class="form-control" placeholder="Prodcut Name">
                                </div>
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                     <textarea required name="pdet" class="form-control" placeholder="Product Details..."></textarea>
                                </div>
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                    <input type="file" required name="image" accept=".png, .jpg, .jpeg" class="form-control">
                                </div>

                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                    <select class="form-control" required name="mname">
                                  ...
                                  ?>
                                  <option value="<?php echo $category["Market_Name"];?>"><?php echo $category["Market_Name"];?></option>
                                  <?php
                                      endwhile;
                                  ?>
                                    </select>
                                ...
                                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                    <select class="form-control" required name="stat">
                                ...
                                <div class="col-lg-3 col-md-6 col-sm-3 col-xs-12 text-center">
                                    <button type="submit" name="addp" id="" class="btn btn-light btn-radius btn-brd grd1 btn-block">Add</button>
                                </div>
                            </fieldset>
                        </form>
                    </div>
                        ...
                        <form class="row" action="insertion.inc.php" name="contactform" method="post" enctype="multipart/form-data">
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                                    <select class="form-control" required name="pid">
                                     ...
                                     <?php
                                      $con = mysqli_connect("localhost","root","","flea_market");
                                      $sql = "SELECT * FROM `products` WHERE `User_ID` = '$filter'";
                                      $all_categories = mysqli_query($con,$sql);
                                      while ($category = mysqli_fetch_array(
                                              $all_categories,MYSQLI_ASSOC)):;
                                  ?>
                                ...
                                <br>
                                <div class="col-lg-3 col-md-6 col-sm-3 col-xs-12 text-center">
                                    <button type="submit" name="upp" id="" class="btn btn-light btn-radius btn-brd grd1 btn-block">Update</button>
                                </div>
                            </fieldset>
                        </form>
                    </div>
